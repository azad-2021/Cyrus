...
if(isset($_POST["Data"]) and  isset($_POST["branch"]))
{
	...
	$Branch=$obj = json_decode($_POST["branch"]);
	...
	for ($i=0; $i < count($Branch); $i++) { 
		...
		for ($i=0; $i < count($OrderIDA); $i++) { 
			...
			if ($Type=='Order') {
				$query="SELECT OrderID, BranchName, Discription FROM orders join branchdetails on orders.BranchCode=branchdetails.BranchCode
				WHERE OrderID=$OID";
			}else{

				$query="SELECT ComplaintID, BranchName, Discription FROM complaints join branchdetails on complaints.BranchCode=branchdetails.BranchCode
