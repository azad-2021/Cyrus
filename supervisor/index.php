...
WHERE demandbase.StatusID=1 and districts.SubControllerID=$EXEID order by DateOfInformation";
...
WHERE (TotalBilledValue-ReceivedAmount)>1 and Cancelled=0 and districts.SubControllerID=$EXEID and BankName!='Cyrus'";
...
while($rowE=mysqli_fetch_assoc($resultTech)){
 ...
 join cyrusbackend.`cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
 WHERE AssignDate is not null and Attended=0 and EmployeeCode=$EmployeeID and districts.SubControllerID=$EXEID";
 $result=mysqli_query($con,$query);
 $row = mysqli_fetch_array($result);
                ...
                <div class="table-responsive container">
                  <table class="table text-start align-middle table-bordered table-hover mb-0">
                    ...
                    <tbody>
                      ...
                      join districts on branchdetails.Address3=districts.District
                      join `cyrus regions` on districts.RegionCode=`cyrus regions`.RegionCode
                      WHERE districts.SubControllerID=$EXEID and AssignDate is null
