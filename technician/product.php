...
$Sub=0;
  ...
  if(isset($_POST['submit'])){ 
    ...
    $file_name = $_FILES['InstPaper']['name'];
    //$file_name = 'data';
    $file_size =$_FILES['InstPaper']['size'];
    $file_tmp =$_FILES['InstPaper']['tmp_name'];
    $file_type=$_FILES['InstPaper']['type'];
    $tmp = explode('.', $_FILES['InstPaper']['name']);
    $file_ext = strtolower(end($tmp));    
    $newfilename=$approvalID.".".$file_ext;         
    ...
    if (empty($errors)==true) {
      ...
      $Upload=move_uploaded_file($file_tmp,"InstallationPaper/".$newfilename);
